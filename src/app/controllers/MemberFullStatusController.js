...
class MemberFullStatusController {
  async index(req, res) {
    ...
    const membersFullStatus = members.map(async member => {
      const statusPositionCount = member.position_id
        ? await StatusPosition.count({
            where: { position_id: `${member.position_id}` },
          }).then(item => {
            return item;
          })
      ...
      const statusMemberCount = await StatusMember.count({
        where: { member_id: member.id, is_removed: false },
      }).then(item => {
        return item;
      });
      ...
      return {
        ...
        personalStatus: statusMemberCount,
      };
    });

    const results = await Promise.all(membersFullStatus);
